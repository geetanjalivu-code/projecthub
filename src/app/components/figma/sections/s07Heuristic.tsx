            ...
            {criticals.map((sc, i) => {
              ...
              return (
                <div key={i} className="flex items-center gap-3 py-2 border-b border-border last:border-0">
                  <span className={`text-xs px-2 py-0.5 rounded-full border ${sc.severity === 4 ? 'bg-destructive/10 text-destructive border-destructive/30' : 'bg-warning/10 text-warning border-warning/30'}`}>
              ...
              <div className="px-5 py-3 border-b border-border bg-muted flex items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-4 shrink-0">
                  ...
                  <div className="flex items-center gap-1">
                    {[1,2,3,4,5].map(n => (
                      <button key={n} onClick={() => updScore(i, 'score', sc.score === n ? 0 : n)}
                        className={`w-7 h-7 rounded border text-xs transition-all ${sc.score >= n ? `bg-primary text-primary-foreground border-primary` : 'bg-muted text-muted-foreground border-border hover:border-primary'}`}>
                        {n}
                      </button>
                    ))}
                  ...
                  {/* Severity */}
                  <div className="flex items-center gap-1">
                    {SEVERITY_LABELS.map((sev, si) => (
                      <button key={si} onClick={() => updScore(i, 'severity', sc.severity === si ? 0 : si)}
                        title={sev}
                        className={`w-2 h-6 rounded-sm transition-all border ${sc.severity >= si && si > 0 ? (si >= 3 ? 'bg-destructive border-destructive' : si === 2 ? 'bg-warning border-warning' : 'bg-success border-success') : 'bg-muted border-border hover:border-primary'}`} />
                    ))}
                    <span className={`text-xs ml-1 ${SEVERITY_COLORS[sc.severity]}`}>{SEVERITY_LABELS[sc.severity]}</span>
                  </div>
                  {/* Edit / Done */}
                  <button onClick={() => setEditingIdx(editing ? null : i)}
                ...
                {editing ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <textarea rows={3} value={sc.issue} onChange={e => updScore(i, 'issue', e.target.value)}
                    ...
                    <div>
                      ...
                      <textarea rows={5} value={sc.notes} onChange={e => updScore(i, 'notes', e.target.value)}
